...
public class GridEnvironment_DDPG : MonoBehaviour
{
    public float reward;
    public bool done;
    public int maxSteps;
    ...
    AgentDDPG addpg;
    public int batch_capacity = 200;
    public int batch_size = 32;
    public int updater = 200;
    public int input_size = 1;
    ...
    public virtual void Step()
    {
        ...
        float losslabel = 0;
        float[,] s = new float[1, 1];
        reward = 0;
        GameObject actor =GameObject.FindGameObjectWithTag("agent");
        s[0, 0] = (10 * actor.transform.position.x) + actor.transform.position.z;
       
        float[,] action = addpg.Predict(s);

        string actions = ""+Mathf.Floor(action[0,0]);
        MiddleStep(actions);

        if (istraining == true)
        {
            float loss = addpg.Train(s, action, reward);
            if (loss != 0)
                losslabel = loss * loss;
        }
        acceptingSteps = true;
